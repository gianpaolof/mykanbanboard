...
PROMPTEOF
    ...
    if [[ "$use_modern_cli" == "true" ]]; then
        ...
        if $CLAUDE_CODE_CMD $CLAUDE_EXTRA_FLAGS --output-format "$CLAUDE_OUTPUT_FORMAT" --allowedTools "${CLAUDE_ALLOWED_TOOLS[@]}" < "$CONVERSION_PROMPT_FILE" > "$CONVERSION_OUTPUT_FILE" 2> "$stderr_file"; then
            cli_exit_code=0
        else
            cli_exit_code=$?
        fi
        ...
        if $CLAUDE_CODE_CMD $CLAUDE_EXTRA_FLAGS < "$CONVERSION_PROMPT_FILE" > "$CONVERSION_OUTPUT_FILE" 2> "$stderr_file"; then
            cli_exit_code=0
        else
            cli_exit_code=$?
        fi
