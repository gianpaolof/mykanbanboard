...
if command -v timeout &> /dev/null; then
    ...
    # Fallback: no timeout (run indefinitely)
    TIMEOUT_CMD=""
fi
...
setup_tmux_session() {
    ...
    # Split window horizontally to create monitor pane on the right
    tmux split-window -h -t "$session_name" -c "$(pwd)"

    # Start monitor in the right pane (pane 1)
    if command -v ralph-monitor &> /dev/null; then
        tmux send-keys -t "${session_name}.1" "ralph-monitor" Enter
    else
        tmux send-keys -t "${session_name}.1" "'$ralph_home/ralph_monitor.sh'" Enter
    fi

    # Start ralph loop in the left pane (pane 0, exclude tmux flag to avoid recursion)
    local ralph_cmd
    ...
    tmux send-keys -t "${session_name}.0" "$ralph_cmd" Enter

    # Focus on left pane (main ralph loop)
    tmux select-pane -t "${session_name}.0"

    # Set window title
    tmux rename-window -t "$session_name" "Ralph: Loop | Monitor"
...
reset_session() {
    ...
    # Reset exit signals to prevent false completion detection on next run
    echo '{"test_only_loops": [], "done_signals": [], "completion_indicators": []}' > "$EXIT_SIGNALS_FILE"
...
build_claude_command() {
    ...
    # Add extra flags (like --dangerously-skip-permissions)
...
execute_claude_code() {
    ...
    if [[ "$use_modern_cli" == "false" ]]; then
        if [[ -n "$TIMEOUT_CMD" ]]; then
            if $TIMEOUT_CMD ${timeout_seconds}s $CLAUDE_CODE_CMD $CLAUDE_EXTRA_FLAGS < "$PROMPT_FILE" > "$output_file" 2>&1 &
            then
                :  # Continue to wait loop
            else
            ...
            # No timeout command available, run without timeout
            if $CLAUDE_CODE_CMD $CLAUDE_EXTRA_FLAGS < "$PROMPT_FILE" > "$output_file" 2>&1 &
            then
                :  # Continue to wait loop
            else
                log_status "ERROR" "❌ Failed to start Claude Code process"
